...
fn process_contract_trace(
    ...
    if contract_check.starts_with("`�`@R") {
        Some(Contract {
            ...
            transaction_hash:  base_64_to_hex(trx.hash),
...
fn process_transaction_trace(trx: eth::TransactionTrace, block: &eth::Block) -> Transaction {
    ...
    Transaction {
        ...
        index: trx.index,
        nonce: trx.nonce,
        max_fee_per_gas: option_bigint_to_number_u64(trx.max_fee_per_gas),
        max_priority_fee_per_gas: option_bigint_to_number_u64(trx.max_priority_fee_per_gas),
        ...
        block_number: block_number,
        timestamp: time_stamp,
        gas_price: option_bigint_to_number_u64(trx.gas_price)
    }
}
...
fn map_block(block: eth::Block) -> Result<BlockHeader, substreams::errors::Error> {
    ...
    Ok(BlockHeader {
        ...
        parent_hash: base_64_to_hex(&header.parent_hash),
        uncle_hash: base_64_to_hex(&header.parent_hash),
        ...
        total_difficulty: option_bigint_to_number_string(header.total_difficulty.clone()) ,
...
fn store_price(transaction_details_list: TransactionList, store: StoreSetProto<Transaction>) {
    for transaction in transaction_details_list.transaction_details_list {
        store.set(transaction.block_number, format!("transaction.id"), &transaction);
    }
}
