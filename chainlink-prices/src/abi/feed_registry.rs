    ...
    pub mod functions {
        ...
        impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetAccessController {
        ...
        impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetFeed {
            fn output(data: &[u8]) -> Result<Vec<u8>, String> {
                Self::output(data)
            }
        }
        ...
        pub struct GetPhase {
            pub base: Vec<u8>,
            pub quote: Vec<u8>,
            pub phase_id: substreams::scalar::BigInt,
        }
        impl GetPhase {
            const METHOD_ID: [u8; 4] = [255u8, 6u8, 1u8, 192u8];
            pub fn decode(
                call: &substreams_ethereum::pb::eth::v2::Call,
            ) -> Result<Self, String> {
                let maybe_data = call.input.get(4..);
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ...
                        ethabi::Token::Uint(
                            ethabi::Uint::from_big_endian(
                                match self.phase_id.clone().to_bytes_be() {
                                    (num_bigint::Sign::Plus, bytes) => bytes,
                                    (num_bigint::Sign::NoSign, bytes) => bytes,
                                    (num_bigint::Sign::Minus, _) => {
                                        panic!("negative numbers are not supported")
        ...
        impl substreams_ethereum::Function for GetPhase {
            const NAME: &'static str = "getPhase";
            fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
                Self::match_call(call)
            }
            fn decode(
        ...
        impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetPhaseFeed {
            fn output(data: &[u8]) -> Result<Vec<u8>, String> {
                Self::output(data)
            }
        }
        ...
        impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetProposedFeed {
            fn output(data: &[u8]) -> Result<Vec<u8>, String> {
                Self::output(data)
            }
        }
        ...
        impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetRoundFeed {
            fn output(data: &[u8]) -> Result<Vec<u8>, String> {
                Self::output(data)
            }
        }
        ...
        impl substreams_ethereum::rpc::RPCDecodable<bool> for IsFeedEnabled {
    ...
    pub mod events {
        ...
        pub struct FeedConfirmed {
            ...
            pub next_phase_id: substreams::scalar::BigInt,
            pub sender: Vec<u8>,
        }
        impl FeedConfirmed {
            const TOPIC_ID: [u8; 32] = [
